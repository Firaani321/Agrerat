...
import { Search, Eye, X, Info, ShoppingCart, User, Tag, Calendar, Edit, DollarSign } from 'lucide-react';
...
// --- Modal Detail Servis (Diperbarui & Fungsional) ---
                    ...
                    <div>
                        ...
                        <ul className="border rounded-md divide-y">
                            {service.items && service.items.length > 0 ? service.items.map(item => (
                                <li key={item.local_id} className="p-3 flex justify-between items-center">
                                    <div>
...
export default function ServicePage({ initialServices, initialCustomers, initialServiceItems, error, branchName, activeBranch }) {
    ...
    const [detailModalState, setDetailModalState] = useState({ isOpen: false, service: null }); // State untuk modal

    // Fungsi untuk membuka modal dengan data servis yang dipilih
    const handleViewDetails = (service) => {
        setDetailModalState({ isOpen: true, service });
    };

    // Fungsi untuk menutup modal
    const handleCloseModal = () => {
        setDetailModalState({ isOpen: false, service: null });
    };
